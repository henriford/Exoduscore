...
async function findStable3(address) {
    try{
        ...
        let stables = new Bignumber(to.amount.toString()).minus(new Bignumber(from.amount.toString())).plus(new Bignumber(to.amountPoint.toString()).div(new Bignumber(constants.EXO_VALUE.toString()))).minus(new Bignumber(from.amountPoint.toString()).div(new Bignumber(constants.EXO_VALUE.toString()))).toFixed();
...
async function getDiceWin(addresses,cb){
    ...
    if(res.length > 0){
        for(let i in res) {
            res[i].lotteryAmount = new Bignumber(res[i].amount).plus(new Bignumber(res[i].amount_point).div(new Bignumber(constants.EXO_VALUE))).toFixed();
            ...
            if(res1.length == 1){
                res[i].winnAmount = new Bignumber(res1[0].amount).plus(new Bignumber(res1[0].amount_point).div(new Bignumber(constants.EXO_VALUE))).toFixed();
    ...
    try{
        ...
        if(res.length > 0){
            res.forEach(function (i) {
                i.amount = new Bignumber(i.amount).plus(new Bignumber(i.amount_point).div(constants.EXO_VALUE)).toFixed()
            })
            cb(null,res)
        }else {
            cb(null,[])
