...
function parseUri(uri, callbacks) {
    var protocol = conf.program || 'exodus';
   // var re = new RegExp('^' + protocol + '-(.+):(.+)$', 'i');
    ...
    if (!ValidationUtils.isValidAddress(address, 'EXO') && !ValidationUtils.isValidAddress(address, 'ETH') &&!ValidationUtils.isValidAddress(address, 'BTC') ) return callbacks.ifError("address " + address + " is invalid");
    objRequest.type = "address";
    objRequest.walletType = walletType;
    objRequest.walletType = ValidationUtils.isValidAddress(address, 'EXO') ? 'EXO' : ValidationUtils.isValidAddress(address, 'ETH') ? 'ETH' : 'BTC';
