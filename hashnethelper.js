...
// const rpc = require("./Hashnet").one.exodus.rpc;
...
class HashnetHelper {
    ...
    static async sendMessageTry(unit) {
        try {
            ...
            //往共识网发送交易
            let result = await webHelper.httpPost(getUrl(config.URL.EXO_TRANSACTION_URL, '/v1/sendmsg'), null, buildData({message}));
    ...
    static async getTransactionHistory(address, tableIndex, offset, sysTableIndex,sysOffset ) {
        ...
        try {
            ...
            let type = [1, 2];
            //从共识网拉取交易记录
            let resultMessage = JSON.parse(await webHelper.httpPost(getUrl(config.URL.EXO_TRANSACTION_getURL, '/v1/gettransactionlistnew'), null, buildData({
                address,
                tableIndex,
                sysTableIndex,
                offset,
    ...
    static async getHashInfo(hash) {
        ...
        try {
            ...
            //     throw new Error('network error, please try again.');
            // }
            let result = await webHelper.httpPost(getUrl(config.URL.EXO_TRANSACTION_getURL, '/v1/gettransaction'), null, buildData({hash}));
    ...
    static async getAccountInfo (address) {
        ...
        let localfullnode = config.URL.EXO_TRANSACTION_getURL;
        let retry = 3;
        try {
