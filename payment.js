...
async function sendTransactions(opts, cb){
    try{
        ...
        let localfullnode = config.URL.EXO_TRANSACTION_URL;
...
async function sendTransactionToOtherServer(data, cb){
    try{
        ...
        let obj =
            {
                //localfullnode_list: data.localfullnode_list,    //发送到exodus的local full node
                localfullnode_list: [config.URL.EXO_TRANSACTION_URL],    //发送到exodus的local full node
...
let getRate = async (cb) =>{
    try{
        let res = JSON.parse(await webHelper.httpGet(config.URL.EXO_RATE, header,null));
        if(res.code == 0){
