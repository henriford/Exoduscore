...
var my_device_hashnetseed_url = conf.URL.EXO_SEED;//局域网IP
...
async function getInfo(address) {
    addresses = [];
    let result = await db.toList('select a.* from my_addresses a where a.address = ?', address);
    pubKey = JSON.parse(result[0].definition)[1].pubkey;
    walletId = result[0].wallet;
    addresses.push(result[0].address);
...
function decryptPackage(objEncryptedPackage) {
    ...
    /**
     *注释此段错误，提示用户多个设备使用同一个助记词创建钱包，导致设备一致
     */
    else {
        ...
        eventBus.emit('sameDeviceAddress',my_device_address);
        return null;
    }
...
function handlePairingMessage(json, device_pubkey, callbacks) {
    ...
    db.query(
        ...
        "SELECT is_permanent FROM pairing_secrets WHERE pairing_secret IN(?,'*')  ORDER BY (pairing_secret=?) DESC LIMIT 1",
...
function getNodeList(callback){
    webHelper.httpPost(my_device_hashnetseed_url + '/v1/relay/list', null, {'data': ''}, callback);
}
